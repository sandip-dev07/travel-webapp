...
export default function Header() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (tripsRef.current && !tripsRef.current.contains(event.target as Node)) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 flex items-center justify-between px-24 py-5 md:px-36 transition-all duration-300 bg-white z-50 ${showBorder ? 'border-b' : ''}`}>
      <div className="flex items-center">
        <Link href="/" className="mr-6">
          <Image src="/tripadvisor-logo.svg" alt="logo" width={150} height={50} />
        </Link>
      </div>
      <div className="hidden md:flex items-center space-x-3">
        ...
        <Link href="#" className="text-base font-semibold px-4 py-2 rounded-full hover:bg-gray-100 transition-colors">
          Review
        </Link>
        <Link href="#" className="text-base font-semibold px-4 py-2 rounded-full hover:bg-gray-100 transition-colors">
          More
      ...
      <div className="flex items-center space-x-4">
        ...
        <button className="bg-black text-white text-base font-semibold px-3 py-1 rounded-full hover:bg-gray-800 transition-colors">Sign in</button>
      </div>
    </header>
  )
}
